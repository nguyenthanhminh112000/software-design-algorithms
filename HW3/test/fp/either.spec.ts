...
describe('Either algebraic type', () => {
  ...
  describe('map', () => {
    it('maps the right value from A to B', () => {
      ...
      expect(toName(user)).toStrictEqual(right('Gregor'));
    });
  });

  describe('getOrElse', () => {
    const showContent = getOrElse(
      (redirectURL) => `Sorry, but content was moved to ${redirectURL}`
    );
    ...
    it('returns content of onLeft in case of left', () => {
      ...
      expect(showContent(content)).toBe(
        'Sorry, but content was moved to https://example.com/moved-content'
      );
    });
  });
...
${data
  .map((user, i) => `[${i}] ${user.name} | LEVEL: ${user.level}`)
  .join('\n')}
`
    );
...
`);
    ...
    it('returns content of onLeft in case of left', () => {
      ...
      expect(getListOfUsers(response503)).toBe(
        'forbidden: You have no rights to see this list'
      );
    });
  });

  describe('ap', () => {
    const concat = (list: Array<number>) => (list2: Array<number>) =>
      list.concat(list2);

    it('apply parameters to the 2-ary function', () => {
      expect(
        pipe(right(concat), ap(right([1, 2, 3])), ap(right([4, 5, 6])))
  ...
  describe('flatten', () => {
    ...
    const getList = (user: User): Either<string, List> =>
      user.role === 'admin'
    ...
    const fetchUserError = (): Either<string, User> =>
      left('cannot fetch user');

    it('flatten the value', () => {
      const admin = fetchUserAdmin();
      const user = fetchUser();

      expect(pipe(admin, map(getList), flatten)).toStrictEqual(
        right([{ payload: 'secret info 1' }, { payload: 'secret info 2' }])
      );

      expect(pipe(user, map(getList), flatten)).toStrictEqual(
        left('This information is only for admins')
      );

      expect(pipe(fetchUserError(), map(getList), flatten)).toStrictEqual(
        left('cannot fetch user')
      );
    });
  });
  ...
  describe('fromPromise', () => {
    it('returns right or left instance based on promise result', () => {
      ...
      return Promise.all([fromPromise(resolved), fromPromise(rejected)]).then(
        ([resolved, rejected]) => {
