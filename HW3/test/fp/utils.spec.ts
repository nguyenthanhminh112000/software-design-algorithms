...
describe('utils', () => {
  ...
  it('matcher performs pattern matching', () => {
    ...
    const greetAdmiral = (m: Member) =>
      `Good day admiral ${m.name}! Glad to see you here!`;
    ...
    expect(greetMember({ name: 'Raul', role: 'captain' })).toBe(
      'Welcome aboard captain Raul!'
    );
    expect(greetMember({ name: 'Raul', role: 'admiral' })).toBe(
      'Good day admiral Raul! Glad to see you here!'
    );
    expect(greetMember({ name: 'Raul', role: 'sailor' })).toBe(
      'Hurry up and start working, Raul!'
    );
    expect(greetMember({ name: 'Raul', role: 'peasant' })).toBe(
      'Who are you? Raul? You should not be here'
    );
  });
